...
Let's do hyper parameter tuning to achieve better performance. Let's use the kerlab SVM for the classification. First predict using the default hyperparameters. Also, at this time, let's change the target variabe to 'catch rate. First let's create a new task for the regression analysis

```{r}
names(pokemon)
svm.task=makeRegrTask(data = pokemon,target = "Catch_Rate")
svm.learner=makeLearner("regr.ksvm")
listMeasures(svm.task)
svm.measures=list(rsq,mse,rmse,timeboth)
svm.resample.expt=resample(learner = svm.learner,task = svm.task,resampling = rf.resamp,measures = svm.measures)
svm.resample.expt$aggr
```

Now, let's get all the hyperparameters for KSVM

```{r}
getParamSet("regr.ksvm")
```

Here the regularization is represented by the C hyperparameter which defaults to 1. Let's tell MLR to randomly pick c value between 2^-5 to 2^5
...
#create a random search in the space with 100 iterations
...
Lets investigate the results of our tuning

```{r}
data=generateHyperParsEffectData(hypertunesvm)
data$data
plotHyperParsEffect(data,x="C",y="mse.test.mean")
```

Let us see how long it took us to find that optimal value
